...
export const DEFAULT_PROJECT = {
  ...
  evaluation:
    'Research Questions and Hypotheses: RQ1: Does reinforcement learning with dense, process-level rewards improve exact-match accuracy on multi-step math word problems relative to supervised fine-tuning alone? Hypothesis: Process-level reward signals will yield higher exact-match accuracy and more stable training than outcome-only RL.\nMetrics and Benchmarks: Exact-match accuracy on a standard grade-school math word-problem benchmark; training stability via KL divergence and reward variance; sample efficiency measured by steps to convergence.\nComparative Baselines: (1) supervised fine-tuning only; (2) classic PPO-style RL with outcome-only rewards; (3) proposed GRPO configuration with process rewards and optional code verification.\nAblations and Sensitivity Analysis: Remove individual reward components, disable curriculum scheduling, and vary self-consistency sample counts.\nAnalysis Plan: Report learning curves, error categories, and reproducibility artifacts including logged hyperparameters and evaluation scripts.\nSuccess Criteria: Measurable accuracy gain over the supervised baseline with stable training and documented ablations isolating contributing design choices.',
  timeline:
    'Expected results: A reproducible training codebase, experiment logs, and ablation report documenting how reward design and curriculum choices affect math reasoning accuracy relative to the stated research questions.\nMilestone 1 (Weeks 1--3): Reproduce the data pipeline and supervised fine-tuning baseline with documented exact-match scores to establish the comparison point for RQ1.\nMilestone 2 (Weeks 4--6): Implement dense process rewards and sandboxed Python verification; validate reward signals on a held-out development set to test the process-reward hypothesis.\nMilestone 3 (Weeks 7--10): Train GRPO with curriculum scheduling and multi-sample rollouts; monitor stability via KL drift and compare learning dynamics against the PPO-style baseline.\nMilestone 4 (Weeks 11--12): Run self-consistency inference and hyperparameter sweeps; compare against the PPO-style baseline under the evaluation metrics.\nMilestone 5 (Weeks 13--14): Conduct final benchmark evaluation, error analysis, and write-up that directly answers whether process-reward RL improves exact-match accuracy over supervised fine-tuning alone.',
  resources:
